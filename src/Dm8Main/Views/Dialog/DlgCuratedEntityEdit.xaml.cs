...
namespace Dm8Main.Views.Dialog
{
    ...
    /// </summary>
    public partial class DlgCuratedEntityEdit : MetroWindow, IClosableWindow
    {
        ...
        public DlgCuratedEntityEditViewModel ViewModel => (this.DataContext as DlgCuratedEntityEditViewModel)!;

        public DlgCuratedEntityEdit()
        {
            this.InitializeComponent();
            DialogServiceViews.SetIsRegistered(this, true);
            this.RestoreState(Properties.Settings.Default.DlgCuratedEntityAddSettings);
        }


        private void DlgCuratedEntityAdd_OnLoaded(object sender, RoutedEventArgs e)
        {
        ...
        private void DlgCuratedEntityAdd_OnClosing(object? sender, CancelEventArgs e)
        {
            Properties.Settings.Default.DlgCuratedEntityAddSettings = this.StoreState();
        ...
        {
            for (int i = 0; i<this.ViewModel.NumSelectablePages && i < this.HamburgerMenuControl.Items.OfType<HamburgerMenuItem>().Count(i => i.IsVisible); i++)
            {
                (this.HamburgerMenuControl.Items.GetItemAt(i) as HamburgerMenuItem).IsEnabled = true;
            }

            for (int i = this.ViewModel.NumSelectablePages; i < this.HamburgerMenuControl.Items.OfType<HamburgerMenuItem>().Count(i => i.IsVisible); i++)
            {
        ...
        private void SelectAll_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            if (c != null)
            {
                ...
                DlgCuratedEntityEditViewModel dm = ((DlgCuratedEntityEditViewModel)this.DataContext);
